...
You can enhance your diagrams using the massive collection of open-source Excalidraw libraries (icons, logos, complex shapes) available locally in `excalidraw-libraries`.

We have a dedicated tool to help you search and inject these library elements: `references/excalidraw_lib.py`

### 1. Search for a Library
Find libraries containing relevant icons (e.g., "AWS", "database", "logos"):
```bash
uv run python .claude/skills/excalidraw-diagram/references/excalidraw_lib.py search "aws"
```

### 2. Inspect a Library
Once you find a library, inspect it to see the items it contains:
```bash
uv run python .claude/skills/excalidraw-diagram/references/excalidraw_lib.py inspect slobodan/aws-serverless.excalidrawlib
```
*Note: This will list the items by index and try to show any text they contain so you can guess which one is the Lambda icon, S3 icon, etc.*

### 3. Extract and Inject
Extract the raw JSON elements for a specific item, automatically translating them to the specific `x` and `y` coordinates you want in your diagram:
```bash
uv run python .claude/skills/excalidraw-diagram/references/excalidraw_lib.py extract slobodan/aws-serverless.excalidrawlib 4 --x 150 --y 300
```
Then simply paste the resulting JSON objects into the `elements` array of your `.excalidraw` file!
